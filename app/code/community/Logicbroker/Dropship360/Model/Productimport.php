...
class Logicbroker_Dropship360_Model_Productimport {
	...
	protected $allowedAttribute = array('name','short_description','description','weight','price','msrp','manufacturer','action','upc','manufacturer_product_number');
	protected $preDefineAttribute = array (
			'status' => 2,
			'visibility' => 1,
			'tax_class_id' => 0 
	);
	protected $arrMultiInsert = array ();
	...
	public function getAttributeInfo($name, $value) {
		...
		//$attr = $_product->getResource ()->getAttribute ( $name );
	...
	public function insertAttributeData($lastId, $data, $sku) {
		...
		foreach ( $data as $key => $val ) {
			...
			if ($key == 'int') {
				...
				$i = 1;
				//echo "INSERT INTO $t (entity_type_id,attribute_id,store_id,entity_id,value) VALUES " . $insvalues ;
	...
	protected function prepareAttributeInsertQuery($lastId, $data, $sku) {
		...
		$tableName = $this->prePopulatedAttribute ['attribute_detail'] [$sku] [$data ['name']] ['table_name'];
		...
		foreach ( $productAttributeArray as $product => $data ) {
			...
			switch (strtolower($data['extra']['action'])) {
				...
				case 'a':
					...
					//$this->updateProductQuantity($prodcutId,$data['extra']['quantity'] ,$data ['sku']);
				break;
				
				case 'u':
					if($this->isSkuExsist($data ['sku']))
						$result[$data ['sku']] = ($this->updateProduct ( $data ['sku'], $data ['attributeData'],$data['extra']['quantity'] )) ? array('Product Updated Successfully'):array('Unspecified Error While Updating Product');  
					else 
	...
	protected function addProduct($data) {
		...
		if ($lastId && $lastId > 0) {
			...
			// print_r($arrMultiInsertQuery);die();
	...
	public function updateProduct($sku, $attributes,$qty) {
		$dataAttribute = array ();
		$sucees = false;
		...
		try {
			$product->save ();
			$sucees = true;
			//$this->updateProductQuantity($product->getId(),$qty);
		} catch ( Exception $e ) {
			Mage::logException($e);
	...
	protected function updateProductQuantity($prodcutId = '',$qty,$sku = ''){
		
		if($sku){
			$prodcutId = Mage::getModel('catalog/product')->getIdBySku($sku);
		}
		
		if($prodcutId){
		$stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($prodcutId);
			if (!$stockItem->getId()) {
				$stockItem->setData('product_id', $prodcutId);
