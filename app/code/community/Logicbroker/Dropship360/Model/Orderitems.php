...
class Logicbroker_Dropship360_Model_Orderitems extends Mage_Core_Model_Abstract
{
	...
	public function prepareOrderItemData($item){
		...
		//echo $collectionVendor->getSelect ().'<hr>';
		//die;
		//$collectionVendor->getFirstItem ();
	...
	public function setItemData($orderItemInstance,$status,$item,$vendorCode,$vendorCost,$vendorSku){
	...
	public function updateLbVendorInvenory($vendorCode,$productSku,$qtyInvoiced) {
		...
		//$filedData = $inventory->load ( $vendorCode, 'lb_vendor_code' );
	...
	protected function _changeAllItemStatus($orderId,$itemId,$updatedBy = 'user'){
	...
	public function setSourcingOrderStatus($data){
		...
		switch($itemStatus)
		{
			...
			case in_array('Backorder',$itemStatus) :
					$orderCollection->setStatus(Mage::getStoreConfig(Logicbroker_Dropship360_Model_Observer::XML_PATH_LOGICBROKER_ORDER_BACKORDERED));       //done
			...
			case (in_array('Transmitting', $uniqueArray)) :
				$orderCollection->setStatus(Mage::getStoreConfig(Logicbroker_Dropship360_Model_Observer::XML_PATH_LOGICBROKER_ORDER_AWAITING_TRANSMISSION));       //done
				$orderCollection->setState('processing');
			...
			default : 
				$orderCollection->setStatus(Mage::getStoreConfig(Logicbroker_Dropship360_Model_Observer::XML_PATH_LOGICBROKER_ORDER_SOURCING_COMPLETE));       //done
				$orderCollection->setState('processing');
